...
class VentasFabricante extends PageController
{
    ...
    {
        parent::__construct('VentasFabricante');
        $this->setTemplate('ventas_fabricante');
    ...
    public function privateCore(&$response, $user, $permissions): void
    {
        ...
        if ($cod !== null && $cod !== '') {
            $fab = $this->fabricante->get((string) $cod);
            if ($fab) {
                $this->fabricante = $fab;
            } else {
        ...
        if ($this->fabricante->codfabricante !== null) {
            $offset = $this->request->query->getInt('offset', 0);
            $this->articulos = $this->fabricante->get_articulos($offset);
        }
    }
    ...
    private function eliminarFabricante(Request $request): void
    {
        ...
        if ($fab->delete()) {
            ...
            $this->redirect('ventas_fabricantes');
            return;
        }
