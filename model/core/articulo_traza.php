...
class articulo_traza extends \fs_model
{
    ...
    public function all_from_linea($tipo, $idlinea, $order = 'id DESC')
    {
        ...
        // Solo columnas que existen en el modelo articulo_traza
        $allowedColumns = [
            'idlalbventa', 'idlalbcompra', 'idlfacventa', 'idlfaccompra'
        ];
        
        if (!in_array($tipo, $allowedColumns, true)) {
            return [];
        }
        
        // Whitelist de órdenes permitidos
        $allowedOrders = ['id DESC', 'id ASC', 'fecha_entrada DESC', 'fecha_entrada ASC'];
