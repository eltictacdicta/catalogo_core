...
>   console and no flag.
> - **Clause 2 (legacy article/family columns) — DEFERRED to a later release (7.5/7.6).**
...
- [ ] 7.5 **DEFERRED to a later release — not done.** Run the soak: enable `FS_CATALOGO_CARACTERISTICAS_READ_THROUGH` in a staging DB, compare flag-off/flag-on output, and watch `visibilidad_sin_sincronizar` during a JSON import. **Rationale**: clause 2 drops the eight legacy article/family columns, which requires a real production soak window — it cannot be compressed into this release. **Prerequisite already closed:** DEV-18 (JSON import mirror, WU-8.1) and DEV-17 (membership-filter ratification, WU-8.2).
- [ ] 7.6 **DEFERRED to a later release — not done.** Run the gated drop on a pre-drop dump: `runPostSoak($db, true)` once the readiness gates hold (or `dropLegacyArticleFamilyColumns($db)` **then** `dropDeadOpcionalColumns($db)`) — but only **after** the DEV-17 membership filters are rewritten to the feature tables (the ordered prerequisite recorded in the service docblock and enforced as the required attestation). **Rationale**: clause 2 is the half of CAR-15 that needs the soak; it stays the operator path now and will be auto-wired into `Init::upgrade()` in the release that follows the soak, exactly as clause 1 is now (7.8). Never executes at boot today.
...
| Soak | DEFERRED (later release) | clause 2 needs a real production soak window; the feature path is the default and has never been soaked (runner dry-run reports `FEATURE (default)`; the constant is undefined) |
| Gated drop (clause 2) | DEFERRED (later release) | operator step, never executed (live introspection: 12 gated columns still present); will be auto-wired after the soak, exactly as clause 1 is now |
