...
> **STAGED 2026-09-23 (two-release split, maintainer decision).** CAR-15's two clauses ship
> in **two releases**, because clause 2 needs a real soak window and clause 1 does not:
>
> - **Clause 1 (D12 opcional flags) — DELIVERED NOW (7.8).** Its gate is *only* the CAR-12
>   behaviour-preservation parity test, which is live, and the derived indicator already
>   replaces the flags. It is auto-wired into `Init::upgrade()`, so production needs no
>   console and no flag.
> - **Clause 2 (legacy article/family columns) — DEFERRED to a later release (7.5/7.6, now RELOCATED to `caracteristicas-post-soak-drop`).**
>   It needs the soak first; the drop stays the operator path now and will be auto-wired in
>   the release that follows the soak, exactly as clause 1 is now.
...
- [x] 7.2 GREEN: `dropLegacyArticleFamilyColumns()` implemented — refuses while the legacy read path is **explicitly** selected (`CaracteristicaConfig::legacy_read_explicitly_enabled()`, the emergency opt-out; the feature path is the default, so it runs when the constant is undefined or TRUE); `hasColumn()` before each `ALTER`; `tarif_familia_ext` introspection-guarded no-op; idempotent second run; **NOT wired to `Init::init()`**. **LANDED in WU-4; gate semantics amended by 7.7.**
...
- **7.5 — RELOCATED to `caracteristicas-post-soak-drop` (task C2.1); NOT DONE here.** The soak task was moved out of this change so it can archive cleanly, and is now tracked at `plugins/catalogo_core/openspec/changes/caracteristicas-post-soak-drop/tasks.md`. **Rationale (unchanged)**: clause 2 drops the eight legacy article/family columns, which requires a real production soak window — it cannot be compressed into this release. **Prerequisite already closed:** DEV-18 (JSON import mirror, WU-8.1) and DEV-17 (membership-filter ratification, WU-8.2).
- **7.6 — RELOCATED to `caracteristicas-post-soak-drop` (tasks C2.2–C2.4); NOT DONE here.** The gated-drop task moved with 7.5 to the follow-up change, which also owns the DEV-17 membership-filter rewrite and the clause-2 auto-wiring into `Init::upgrade()` exactly as clause 1 is now (7.8). **Rationale (unchanged)**: clause 2 is the half of CAR-15 that needs the soak; it stays the operator path now and will be auto-wired into `Init::upgrade()` in the release that follows the soak. Never executes at boot today.
- [x] 7.7 RED+GREEN: retire the flag-as-requirement — **the feature path is the DEFAULT and the constant survives only as an emergency opt-out**. `CaracteristicaConfig::read_through()` delegates to the new `legacy_read_explicitly_enabled()` (`defined(READ_THROUGH_FLAG) && constant(...) === false`): undefined selects the feature tables, an explicit `TRUE` keeps them, and only an explicit `FALSE` opts out to legacy. `CaracteristicaColumnDropMigration`'s clause 2 + dead-column gate now refuses on that opt-out (not on the flag being ON), and the `runPostSoak` report key is `legacy_read_explicitly_enabled` with a refusal message naming the opt-out. `tools/run_caracteristica_column_drop.php` reports the new semantics. Docs amended: `design.md` §1 (pinned table + [DECISION]), §8.4, §8.5, §15.2; `specs/README.md` decision 5. **Evidence**: `CaracteristicaValorStoreTest` (undefined/TRUE/FALSE), `CaracteristicaColumnDropTest` (opt-out refusal + default-runs), `CaracteristicaReversibilityTest`; catalogo_core 914/0, tarifario 266/0. **Rationale**: production must need no flag; after the gated drop an undefined constant must still read the feature tables or the catalog silently reads dropped columns.

- [x] 7.8 RED+GREEN: deliver CAR-15 clause 1 automatically — **no console, no flag**. `CaracteristicaColumnDropMigration::migrateIfNeeded(\fs_db2 $db)` is the deploy-time entry point (parity-gated, no soak), wired into `Init::upgrade()` — the version-change path run by `PluginSchemaSynchronizer` — exactly like the sibling boot migrations (`CatalogLegacyTableMigration` / `CaracteristicaBackfillMigration` / `TarifOpcionalExtMigration`), and **never** into `Init::init()`. Clause 1 now refuses under the explicit legacy opt-out (`legacy_read_explicitly_enabled()`), the same emergency-rollback gate clause 2 honours, so the safety property holds for the auto-wired path and the operator path alike; `dropD12OpcionalColumns()` remains the parity-gated engine and `runPostSoak()` keeps its atomic refusal. Idempotent: `hasColumn()` per table, a second run emits no DDL. Clause 2 stays staged (7.5/7.6). **Evidence**: `CaracteristicaColumnDropTest` 26 → **31** tests (opt-out refusal; boot entry drop + idempotency; boot entry refusal; the clause-2 columns untouched; `Init::upgrade()` wiring with `Init::init()` excluded); `CaracteristicaReversibilityTest`'s opt-out test reconciled to `..._blocks_every_gated_clause`; catalogo_core **919/0**, tarifario **266/0**; runner dry run reports 12 gated columns and emits no DDL.

**Acceptance**: CAR-15 clause 1 (delivered, 7.8) + clause 2 + reversibility scenario + the closed-loop entry point (clause 2 staged for a later release).
**Rollback boundary**: 7.8 is a schema DROP on deploy — revert the `Init::upgrade()` wiring and the clause-1 refusal in `CaracteristicaColumnDropMigration`; the columns are re-addable as nullable and re-derivable (7.1b). WU-7's clause 2 is destructive by design; its rollback is a pre-drop dump plus the re-add-nullable/re-derive path. Nothing else in the change depends on WU-7.
...
| Closed-loop entry point | DONE (7.3) | `runPostSoak()` + `POST_SOAK_STEPS` + `pendingVisibilityColumns()`; 8 new entry-point tests in `CaracteristicaColumnDropTest`; runner `tools/run_caracteristica_column_drop.php` |
| Clause 1 delivery (auto-wired) | DONE (7.8) | `CaracteristicaColumnDropMigration::migrateIfNeeded()` called from `Init::upgrade()` (never `init()`); refuses under the explicit legacy opt-out; `CaracteristicaColumnDropTest` 26 → **31**; catalogo_core 919/0 |
| Soak | RELOCATED (follow-up `caracteristicas-post-soak-drop` C2.1) | clause 2 needs a real production soak window; the feature path is the default and has never been soaked (runner dry-run reports `FEATURE (default)`; the constant is undefined) |
| Gated drop (clause 2) | RELOCATED (follow-up `caracteristicas-post-soak-drop` C2.2–C2.4) | operator step, never executed (live introspection: 12 gated columns still present); will be auto-wired after the soak, exactly as clause 1 is now |
| DEV-18 (soak blocker) | DONE (WU-8.1) | `TarifCatalogoJsonImportVisibilityTest` 8/8 |
| DEV-17 (soak blocker) | DONE (WU-8.2) | `TarifCatalogoMembershipFilterRatificationTest` 5/5; the closure **rewrite** remains part of the follow-up change's ordered prerequisite (relocated 7.6 / C2.2) |
...
2. **Clause 1 is parity-gated, NOT soak-gated — confirmed by a RED assertion that had to be corrected.** The first version of `test_a_closed_gate_changes_no_schema_...` asserted `dropD12OpcionalColumns()` returned FALSE with `read_through()` off; the drop service returned TRUE. That is correct per CAR-15 ("D12 drop is gated on parity, not on soak"), so the test now asserts the real contract: with the soak gate closed, clause 1 still runs and only clause 2 + the dead-column cleanup refuse. **Amended 2026-09-23 (7.8):** the finding still holds — clause 1 is parity-gated, needs no soak, and still runs when the flag is undefined (the production default). It now *also* refuses under the **explicit** legacy opt-out (`legacy_read_explicitly_enabled()`), the same emergency-rollback gate clause 2 honours, so `CaracteristicaReversibilityTest::test_the_legacy_opt_out_blocks_every_gated_clause` replaced the old `..._but_not_clause_1` assertion.
...
3. **(a) has an ordering hazard the ratification makes explicit.** The filters *must* stop
   ...
   and in the follow-up change's ordered prerequisite (relocated 7.6) instead of being implicit.
