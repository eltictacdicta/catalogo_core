...
The blocks below are full copies of the **current** canonical requirement and all
its scenarios, edited in place — never partial blocks. They are re-based on the
canonical after the sibling archive `caracteristicas-producto` (`f369faf7`,
flag-driven feature columns), so the feature-column clauses and scenarios it
delivered survive verbatim and are extended, not reverted.
